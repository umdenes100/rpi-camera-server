...
# Install libcamera apps and GStreamer bits
sudo apt install -y \
  libcamera-apps \
  gstreamer1.0-tools gstreamer1.0-plugins-base \
  gstreamer1.0-plugins-good gstreamer1.0-plugins-bad gstreamer1.0-plugins-ugly \
  gstreamer1.0-libav ffmpeg ufw

# Enable camera interface if not already enabled (Bullseye/RPi firmware)
...
#!/bin/bash
set -euo pipefail
# Script to send 1080p@50fps H.264 stream to $RECEIVER_IP:$RECEIVER_PORT
...
# Allow outgoing UDP packets to specified IP/port
sudo ufw allow out to "$RECEIVER_IP" port "$RECEIVER_PORT" proto udp
